...
export function parseDeckConfig(value: unknown): DeckConfig {
  ...
  if (typeof routing.model !== "string" || !/^jev-(?:latest|\d+\.\d+\.\d+)$/.test(routing.model)) throw new Error("routing.model 必须是 Jev 模型 ID，例如 jev-1.13.0。");
...
export async function writeDeckConfig(changes: Partial<DeckConfig>): Promise<void> {
  const file = deckConfigPath();
  await withFileMutationQueue(file, () => withDiskLock(`${file}.lock`, async () => {
    const config = parseDeckConfig({ ...readDeckConfig(), ...changes });
