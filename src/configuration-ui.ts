...
import { editJevConfig } from "./jev-ui.ts";
...
export function registerConfiguration(pi: ExtensionAPI, changed: (ctx: ExtensionContext) => void) {
  ...
  const openConfiguration = async (args: string, ctx: ExtensionContext): Promise<void> => {
    ...
    if (!selected) {
      const items = [{ id: "jev", label: "Jev：模型、密钥与连接检查" }, { id: "global", label: "全局：派遣开关和默认时限" }, { id: "new", label: "描述需求，自动创建 Agent" }, ...agents.map((agent) => ({ id: agent.id, label: `${agent.name} · ${agent.source} · ${agent.id}` }))];
      const choice = await selectMenu(ctx, "Agent 配置", items.map((item) => ({ value: item.id, label: item.label })));
      if (!choice) return;
      selected = choice;
    }
    if (selected === "jev") return editJevConfig(ctx, () => pi.getThinkingLevel?.() ?? "off");
  ...
  pi.registerCommand("agent-config", {
    description: "选择并编辑 Jev、模型、工具或提示词：/agent-config [jev|global|角色ID] [--raw]",
